...
To parse biological structure in the data beyond mere species lists, **ecostructure** allows phylogenetic, geographic, and morphological data to be incorporated in the clustering framework, producing clusters that account for the similarity of species. This allows comparisons of structure according to variation along different axes of diversity. In this framework, we call the clusters *motifs*.
...
#source("R/ecos_fit_dev.R")
#fit <- ecos_fit(species_counts, K = 2)
```

```{r message=FALSE, warning=FALSE}
fit <- ecos_fit_dev(species_counts, K = 3, sparse = F,
                    num_iter_main = 1000, 
                    num_iter_refine = 1000)
```

The function outputs a lower rank representation of the data in `species_counts`, according to the value of K chosen by the user. Above, we chose to examine the primary partition of the data into 2 motifs by setting `K` = 2. The method in `fastTopics` takes advantage of a two step model fit, and `num_iter_main` and `num_inte_refine` can each be modified to control the number of iterations the underlying algorithm will run before convergence during each step. These may need to be modified if the input data matrix is large. The input data matrix can be transformed to a `sparse.matrix` object if `sparse == T` - this is useful if the matrix contains mostly 0 abundances/absences and compute times are burdensome.  
...
We use the function `ExtractTopFeatures`, supplied by `ecostructure` but originally from the R package [CountClust](https://github.com/kkdey/CountClust/blob/master/R/ExtractTopFeatures.R), to compute and record the top contributing species to each species abundance motif.
...
```{r echo=TRUE, eval=FALSE}
pres_ab_fit <- ecos_fit_dev(as.matrix(indian_birds_pa), K = 3)
```
...
ecos_plot_pie(omega = australia_model$omega,
              coords = australia_birds$latlong[,c(2,1)], 
              long_lim = c(110,160),
              lat_lim = c(-50,-10),
...
We can generate maps for all of the dispersion fields using the function `dsp_plot_map`. We provide the dispersion fields for our 38 Himalayan community surveys, `dispersion_field_ex`, as a list of matrices.
...
himalayan_geo_motifs$motif_maps[[2]]
```
...
gdb_global_birds <- sf::st_read("/Users/alex/projects/BL_geodatabase/BOTW_2022_2/BOTW.gdb")

## Then run the following commands to generate the presence absence matrix
```

```{r}
#recast_geom <- st_geometry(gdb_global_birds[st_geometry_type(gdb_global_birds) %in% c("MULTISURFACE"),]) 
#recast_index <- as.numeric(rownames(gdb_global_birds[st_geometry_type(gdb_global_birds) %in% c("MULTISURFACE"),]))
#for (i in 1:length(recast_index)){
#  st_geometry(gdb_global_birds)[[recast_index[[i]]]] <- st_cast(recast_geom[[i]], "MULTIPOLYGON")
#}
```

With the birds of the world dataset, we focus on the presence of species in known localities and areas of potential and likely extinction after 1500 AD. This allows us to study evolutionary patterns without worrying that human induced extinctions bias the data. We filter the data to include presences where the species is either native or reintroduced to a historically native range, and where the species is resident or breeding.  

```{r}
gdb_filter <- gdb_global_birds %>% 
  filter(presence == 1 | presence == 4 | presence == 5) %>% 
...
```{r echo=TRUE, eval=FALSE}
data("hbw_tyrannidae")
tyrannidae <- hbw_tyrannidea$`Scientific name`
dsp_from_scratch <- dsp_create_from_gdb(gdb_object = gdb_filter,
                      raster_resolution = 1,thresh = 3,
                      raster_latlim = c(-90,90),
                      raster_longlim = c(-180,180),
                      species_feature = "sci_name",
                      species_name = tyrannidae)
```

```{r}
dsp_pres_abs_birds <- dsp_pres_abs_from_gdb(gdb_object = gdb_filter,
                      raster_resolution = 1,thresh = 3,
                      raster_latlim = c(-90,90),
                      raster_longlim = c(-180,180),
                      species_feature = "sci_name")
...
```{r}
disp_from_scratch <- dsp_create_from_pres_abs(
  pres_ab = dsp_pres_abs_birds$pres_ab,
  r_sfs_ag = dsp_pres_abs_birds$raster,
  raster_resolution = 1, thresh = 3,
  raster_latlim = c(-90, 90),
  raster_longlim = c(-180, 180),
  species_feature = "sci_name"
...
The presence absence data is output as `disp_from_scratch$pres_ab`, and we can fit the GoM model using `ecos_fit`.

```{r echo=TRUE, eval=FALSE}
pres_abs_tyrannidae <- dsp_from_scratch$pres_ab
pres_abs_tyrannidae <- pres_abs_tyrannidae[rowSums(pres_abs_tyrannidae > 0) >= 1,]
fit_tyrannidae <- ecos_fit_dev(as.matrix(pres_abs_tyrannidae), K = 10, sparse=F,num_iter_main = 1000, num_iter_refine = 1000)
```


```{r echo=TRUE, eval=FALSE}
ecos_plot_pie(
  omega = fit_tyrannidae$omega,
  lat_lim = c(-65, 90),
  long_lim = c(-180, -25),
  coastline_lwd = 5,
  path = "geostructure_plot_tyrannidae.png",
  image_width = 2000,
  image_height = 2000
  )
```
![](geostructure_plot_tyrannidae.png)
...
```{r}
dsp_fields_matrix_tyrannidae <- dsp_to_matrix(dsp_from_scratch$raster)
```

```{r}
fit_dsp_field_tyrannidae <- ecos_fit_dev(dsp_fields_matrix_tyrannidae, K = 5, sparse = F,
                    num_iter_main = 100, 
                    num_iter_refine = 100)
```
```{r}
tyrannidae_geo_motifs <- dsp_motif(fit_dsp_field_tyrannidae$theta,
  color_ramp = c(
    "black", "darkseagreen3",
    "orange", "red"
  ),
...
tyrannidae_geo_motifs$motif_maps[[1]]
tyrannidae_geo_motifs$motif_maps[[2]]
tyrannidae_geo_motifs$motif_maps[[3]]
tyrannidae_geo_motifs$motif_maps[[4]]
tyrannidae_geo_motifs$motif_maps[[5]]
```
