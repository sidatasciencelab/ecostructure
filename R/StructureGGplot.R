...
StructureGGplot <- function(omega, annotation = NULL,
  ...
  if (is.data.frame(annotation) & length(annotation) > 1) {
  ...
  df_ord <- do.call(
    ...
    lapply(1:nlevels(annotation$tissue_label), function(ii) {
      ...
      if (is_single_sample) {
        ...
        each_sample_order <- apply(temp_df, 1, function(x) {
  ...
  df_mlt <- plyr::rename(df_mlt, replace = c(
    "Var1" = "topic",
    "Var2" = "document"
  ))
    ...
    a <- ggplot2::ggplot(
      ...
      ggplot2::theme(
        ...
        axis.text = ggplot2::element_text(
          size = axis_tick$axis_label_size,
      ...
      ggplot2::scale_y_continuous(
        breaks = seq(0, value_ifl, length.out = ticks_number),
      ...
      ggplot2::scale_x_discrete(
        breaks = as.character((levels(df_mlt$document)[round(tissue_breaks)])),
        labels = names(tissue_breaks)
      ) +
      # Add legend title
